...
public class ShipmentDetails {
	...
	public  ShipmentDetails(double ld) throws SQLException, IOException
	{
		shipmentId=ld;
		
		System.out.println("entered Id is "+shipmentId);
		...
		try{
			...
			String getShipmentDetails="select sd.Shipping_ID,sd.Start_City,sd.Destination,sd.Actual_or_Expected_Delivery_date,sd.Full_Name,"+
											"sd.weight,sd.length,sd.breadth,sd.height,sd.Start_date,sd.ship_status,ISNULL(djk.transit_path,0) "
											+ "transit_path,ISNULL(djk.distance,0) distance,ISNULL(djk.current_city,0) current_city "+ 
			...
			while (shippingRecordDetail.next()) {
				shipingBean.setShipingID(shipmentId);
